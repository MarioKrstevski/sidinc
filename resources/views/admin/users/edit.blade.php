...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="badges">{{ trans('cruds.user.fields.badge') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('badges') ? 'is-invalid' : '' }}" name="badges[]" id="badges" multiple>
                    @foreach($badges as $id => $badge)
                        <option value="{{ $id }}" {{ (in_array($id, old('badges', [])) || $user->badges->contains($id)) ? 'selected' : '' }}>{{ $badge }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('wallet') ? 'is-invalid' : '' }}" name="wallet_id" id="wallet_id">
                    @foreach($wallets as $id => $wallet)
                        <option value="{{ $id }}" {{ ($user->wallet ? $user->wallet->id : old('wallet_id')) == $id ? 'selected' : '' }}>{{ $wallet }}</option>
                    @endforeach
                </select>
