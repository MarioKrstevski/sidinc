    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reward_amount') ? 'is-invalid' : '' }}" type="number" name="reward_amount" id="reward_amount" value="{{ old('reward_amount', $task->reward_amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('deadline') ? 'is-invalid' : '' }}" type="text" name="deadline" id="deadline" value="{{ old('deadline', $task->deadline) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('max_enrolled') ? 'is-invalid' : '' }}" type="number" name="max_enrolled" id="max_enrolled" value="{{ old('max_enrolled', $task->max_enrolled) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('importance') ? 'is-invalid' : '' }}" type="number" name="importance" id="importance" value="{{ old('importance', $task->importance) }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Task::ACTIVE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="active_{{ $key }}" name="active" value="{{ $key }}" {{ old('active', $task->active) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('scheduled_at') ? 'is-invalid' : '' }}" type="text" name="scheduled_at" id="scheduled_at" value="{{ old('scheduled_at', $task->scheduled_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple>
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $task->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tags)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $task->tags->contains($id)) ? 'selected' : '' }}>{{ $tags }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
